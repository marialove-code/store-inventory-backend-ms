...
import com.inventory.platform.client.InventoryStockClient;
...
/**
 * 商品管理 Service 实现类（平台服务）。
 * <p>
 * 功能：商品分页查询、新增、修改、删除、上下架、图片上传。
 * </p>
 * <p>
 * <b>跨服务与事务边界：</b>
 * 商品表写操作仍在本服务本地事务内；库存初始化 / 删除前库存校验通过
 * {@link InventoryStockClient} 调用 inventory-service。
 * 本地 {@code @Transactional} <b>管不到</b>远端库存写入——若商品 insert 成功但 initStock 失败，
 * 会出现「有商品无库存」；调用方应感知异常并人工补偿或后续重试 init-stock（接口幂等）。
 * </p>
 *
 * @author 95349
 */
...
public class GoodsProductServiceImpl extends ServiceImpl<GoodsProductMapper, GoodsProduct>
    ...
    /**
     * 库存远程客户端（替代原 InventoryStockMapper 直写）
     */
    ...
    public Result<?> pageProduct(String keyword, String categoryId, String brandId, Integer shelfStatus,
        ...
        // 直接调用XML联查分页（本阶段仍同库联查库存，属渐进拆分技术债）
     ...
     * 新增商品。
     * <p>
     * 本地事务：校验编码 → 插入商品。随后 HTTP 调库存 {@code init-stock}（不在同一事务内）。
     * </p>
     *
     * @param dto 商品参数
     * @return 操作结果
    ...
    public Result<?> addProduct(GoodsProductDTO dto) {
        ...
        // 11. 跨服务初始化库存（远端；本地事务无法回滚该调用）
        StockInitRequest initReq = new StockInitRequest();
        initReq.setGoodsId(product.getId());
        initReq.setGoodsName(product.getProductName());
        initReq.setCategoryName(product.getCategoryName());
        initReq.setStock(product.getStock());
        initReq.setStockWarn(product.getStockWarn());
        initReq.setLockStock(0);
        inventoryStockClient.initStock(initReq);
     ...
     * 单个删除商品（逻辑删除）。
     * <p>
     * 删除前通过库存服务查询账面 stock；stock &gt; 0 不允许删；
     * 库存记录不存在（客户端返回 null）视为可删。
     * </p>
     *
     * @param id 商品ID
     * @return 操作结果
    ...
    public Result<?> deleteProduct(String id) {
        ...
        // 3. 跨服务查询库存快照（含 stock）；null = 无库存记录，允许删除
        Map<String, Object> snapshot = inventoryStockClient.getStockSnapshot(longId);
        if (snapshot != null) {
            Object raw = snapshot.get("stock");
            int stockQty = 0;
            if (raw instanceof Number) {
                stockQty = ((Number) raw).intValue();
            }
            if (stockQty > 0) {
