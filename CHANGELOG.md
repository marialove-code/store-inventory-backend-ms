...
版本独立于单体 `v2.0.0`。Git 标签与里程碑对齐（如 `v3.7.0`）；业务 jar 镜像标签仍可为 `3.6.0`，见各版本说明。

## [3.7.0] — 2026-07-24 · 云原生 + Order CI/CD 收口

- **容器化**：五服务 Dockerfile / compose；本机 compose 探活；服务器 **order** 容器（`order-c` + `deploy-order-docker.sh`）
- **CI/CD**：Gitee Go **Order-CI**（push `ms/dev` 自动编译）+ **Order-CD**（手动确认部署 → jar → 重建容器 → 探活）
- **主机组**：`inventory-prd` + Agent；部署产物名 `order-service-360-jar`（Gitee 无点号限制）
- **文档**：`00-文档导航`、`云原生-总览与实操`、`部署运维-总览`、`CI-CD-order中厂自动化部署`
- 说明：父 POM / jar 文件名仍为 **3.6.0**（与现网镜像一致）；`v3.7.0` 标记工程交付里程碑，后续业务发版再升版本号
