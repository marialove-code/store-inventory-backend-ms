...
**答（速查）：** 详见 **[面试题-09-微服务架构.md](./面试题-09-微服务架构.md) 题 3**（含 AP/CP、自我保护、配置中心、金风 vs 进销存、迁移要点）。

| | Eureka | Nacos |
|--|--------|--------|
| 形态 | 常是一个 Eureka Server 的 Boot 项目 | 独立中间件（解压/Docker 启动） |
| 能力 | 主要做注册发现 | 注册发现 + **配置中心** |
| 一致性 | **AP**，自我保护 | 发现 AP；配置可 CP |
| 现状 | Netflix 不积极演进 | 国内主流，与 SCA 绑定紧 |

本项目用 Nacos Server 2.5.2 + SCA 客户端；金风 2020 前后为 Eureka + Spring Cloud。
